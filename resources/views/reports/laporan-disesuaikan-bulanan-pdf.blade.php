...
<body>
    ...
    <!-- ══════════ JUDUL DOKUMEN ══════════ -->
    <div class="doc-title">
        <h2>REKAPITULASI & BUKU MATRIKS PRESENSI BULANAN</h2>
    </div>
    <div class="doc-nomor">Nomor: {{ $nomorLaporan }}</div>

    <div class="info-row">
        <span>Bulan / Tahun: <strong>{{ $namaBulan }} {{ $tahun }}</strong></span>
        <span>Jumlah Hari: <strong>{{ $daysInMonth }} Hari</strong></span>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th colspan="{{ $daysInMonth }}" style="letter-spacing: 0.5px;">Tanggal Presensi (1 – {{ $daysInMonth }} {{ $namaBulan }})</th>
                <th colspan="4" style="width:70px; background-color:#e2e8f0;">Rekapitulasi</th>
                <th rowspan="2" class="col-persen" style="width:30px;">%</th>
            </tr>
            <tr>
                @for ($d = 1; $d <= $daysInMonth; $d++)
                    @php
                        ...
                        $isWk = $dt->isWeekend();
                    @endphp
                    <th style="width:14px;" class="{{ $isWk ? 'libur-hdr' : '' }}">{{ $d }}</th>
                @endfor
                <th style="width:17px;" title="Hadir">H</th>
                <th style="width:17px;" title="Izin">I</th>
                <th style="width:17px;" title="Sakit">S</th>
    ...
    <div class="no-break">
        <table class="ttd-table" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td class="ttd-cell">
                    <p style="margin:0 0 2px;">Nangtang, {{ \Carbon\Carbon::createFromDate($tahun, $bulan, $daysInMonth)->translatedFormat('d F Y') }}<br><strong>SEKRETARIS DESA NANGTANG</strong></p>
                    <div class="ttd-space"></div>
